...
class HotelBookingExtension(models.Model):
    ...
    # =============================================================================
    # SOBRESCRITURA DE MÉTODO PARA PRESERVAR HORAS ORIGINALES
    # =============================================================================

    def manage_check_in_out_based_on_restime(self):
        """
        SOBRESCRITURA DEL MÉTODO DEL MÓDULO PADRE

        El método original en hotel_management_system normaliza las horas de
        check-in y check-out a la hora configurada en checkout_hours (por defecto 12:00).

        Esta sobrescritura hace que el método NO haga nada, preservando así las horas
        exactas que el usuario ingresó en la reserva.

        Ejemplo:
        - Usuario crea reserva con check-in a las 6:00 PM
        - Sin esta sobrescritura: al confirmar, la hora cambiaría a 12:00 PM (o la hora configurada)
        - Con esta sobrescritura: la hora se mantiene en 6:00 PM

        Si en el futuro se necesita la funcionalidad de normalización de horas,
        se puede eliminar este método o llamar a super().manage_check_in_out_based_on_restime()
        """
        _logger.info(
            "Reserva %s: Preservando horas originales (check_in=%s, check_out=%s). "
            "No se aplicará normalización de horarios.",
            self.id,
            self.check_in,
            self.check_out,
        )
        # No hacemos nada - preservamos las horas originales
        return True
    ...
    def action_confirm_booking(self):
        ...
        if self.status_bar == "initial":
            ...
            # NOTA: NO llamamos a manage_check_in_out_based_on_restime() del módulo padre
            # para preservar las horas originales ingresadas por el usuario.
            # Ese método normaliza las horas a la hora de checkout configurada,
            # pero queremos mantener las horas exactas que el usuario ingresó.
    ...
    def action_no_show(self):
        ...
        if checkin_date and checkin_date >= today:
            raise UserError(
                _("Solo se puede marcar como No Show después de la fecha de entrada.")
            )
    ...
    def update_existing_sale_orders_with_services(self):
        ...
        if (
            not self.order_id
            and self.connected_booking_id
            and self.connected_booking_id.order_id
        ):
        ...
        if not all_orders:
            _logger.warning(
                "No se encontraron órdenes de venta para actualizar en reserva %s (ni en conectadas)",
                self.id,
            )
            return 0
